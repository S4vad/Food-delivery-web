  ...
  return (
    <div className="flex justify-between  w-full h-[120px] p-2  shadow-lg">
      <div className="w-[60%] h-full flex gap-5">
        ...
        <div className="w-[50%] h-fll flex flex-col gap-5">
          ...
          <div className="w-[100px] h-[40px] bg-slate-400 flex rounded-lg overflow-hidden shadow-lg text-green-500  border-green-500 border-2 font-semibold">
            <button className="w-[30%] h-full bg-white flex justify-center items-center hover:bg-gray-300" onClick={()=>{qty>1?dispatch(decrementQty(id)):qty}}> 
            ...
            <button className="w-[30%] h-full bg-white flex justify-center items-center hover:bg-gray-200" onClick={()=>dispatch(incrementQty(id))}>
              +
            </button>
          </div>
        </div>
      ...
      <div className="flex flex-col justify-start items-end gap-4">
        ...
        <RiDeleteBinLine onClick={()=>dispatch(removeItem(id))} className="text-red-300 size-[30px] cursor-pointer" />
      </div>
    </div>
  );
};
